...
export default function LayoutWallet({ children }: { children: React.ReactNode }) {
  ...
  const activeTab = pathname.includes("/topup") ? "topup" : pathname.includes("/transaction") ? "transaction" : "ending-balance";

  return <>
    <div className="flex flex-col gap-6 w-full pb-10">
      {/* Tabs Layout using Link */}
      <div
        className="bg-[#CCF2EB] dark:bg-teal-950/40 p-1 rounded-xl inline-flex gap-1 self-start border border-transparent dark:border-teal-800/20">
        <Link
          href="/wallet/topup"
