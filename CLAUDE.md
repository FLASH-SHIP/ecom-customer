...
- **RSC Safety for i18n**: In Server Components (`layout.tsx`, `page.tsx`), import `defaultLocale` and `locales` directly from `@ecom/i18n`. Do not import from client-only local `./lib/i18n`.
- **Shared UI & Domain**: Consume UI components from `@ecom/ui` and `@ecom/ui/domain`.
- **Type-Safe API**: Consume customer API endpoints via `@ecom/trpc-contract/customer`.
